...
const Navbar = () => {
  ...
  const avatarUrl = user ? `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(user.username)}` : null;
  ...
  return (
    <nav className="relative z-50 bg-black text-gray-200 flex justify-between items-center p-4 h-20 text-sm md:text-[15px] font-medium text-nowrap">
    
      <Link to={"/"}>
      <img src="/logo.png" alt="Netflix Logo" className="w-24  cursor-pointer brightness-125" />
      </Link>
      ...
      <div className="flex items-center space-x-4 relative">
       ...
       <Link to={user ? "/ai-recommendations" : "/signin"}>
        ...
        </Link>) : (<div className="relative text-white"> 
          <img src={avatarUrl} alt="User Avatar" className="w-9 h-9 rounded-full border-2 border-[#e50914] cursor-pointer"
           onClick={() => setShowMenu(!showMenu)}
          />

          {showMenu && (
            <div className="absolute right-0 top-full mt-3 z-50 w-64 bg-[#232323] bg-opacity-95 rounded-lg shadow-lg py-4 px-3 flex flex-col
             gap-2 border border-[#333333]">
